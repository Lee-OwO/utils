...
function a(window) {
  ...
  var _previewImage = function() {
    this.demo = 0 // 不要删
    this.timeOutEvent = 0 // 长按定时器
    this.longTouchTime = 0.7 // 长按触发时间
  ...
  _previewImage.prototype.start = function(obj) {
    //     可优化 todo
    ...
    if (!current) {
      ...
      if (index < 0) {
        index = 0
        console.warn(
          'current isnot on urls,it will be the first value of urls!'
        )
      }
  ...
  _previewImage.prototype.render = function() {
    ...
    this.$text.innerHTML =
      "<span class='previewImage-text-index'>" +
      (this.index + 1) +
      '/</span>' +
      (this.maxLen + 1) //     当前第几张/图片总数
  ...
  _previewImage.prototype.touchStartFun = function(imgitem) {
    ...
    this.timeOutEvent = setTimeout(() => {
      ...
      console.log('你长按了')
      this.longTouch()
    }, this.longTouchTime * 1000)
  }

  _previewImage.prototype.longTouch = function() {
    if (!window.plus) return
    let self = this
    let btns = ['保存图片到手机相册']
    var func = function(e) {
      var index = e.index
      if (index === 1) self.saveImage()
    }
  ...
  _previewImage.prototype.saveImage = function() {
    plus.gallery.save(
  ...
  _previewImage.prototype.moveAction = function(ts, tm) {
    ...
    var x0Offset = tm.x0 - ts.x0
    var y0Offset = tm.y0 - ts.y0
    var time = tm.time - ts.time
    console.log(x0Offset, y0Offset, time)
    if (Math.abs(y0Offset) > 0) {
      //     阻止Ios系统的浏览器环境上下滑动
      event.preventDefault()
    }
    var imgPositionX = imgStatus.x + x0Offset
    var imgPositionY = imgStatus.y + y0Offset
    ...
    if (tm.length == 1) {
      ...
      if (imgStatus.scale > 1) {
        ...
        if (imgPositionY >= allow.y0) {
          ...
          imgStatus.my =
            allow.y0 - imgStatus.y + this.getSlowlyNum(overY, maxWidth)
        } else if (imgPositionY <= allow.y1) {
          //     超过窗口下边界
          this.statusY = 1
          var overY = imgPositionY - allow.y1
          imgStatus.my =
            allow.y1 - imgStatus.y + this.getSlowlyNum(overY, maxWidth)
        } else {
          this.statusY = 2
          imgStatus.my = y0Offset
        }

        //     X方向位移
        if (x0Offset < 0 && imgStatus.x <= -allowX) {
          //     左滑->初始状态到达或超过右边界->图片平滑移动到达条件-切换下一张
          this.statusX = 1
          this.box.m = x0Offset //     更新位移信息
          if (this.index == this.maxLen) {
            //     box到达右边界
            this.box.m = this.getSlowlyNum(x0Offset) //     阻尼效果
          }
          this.translateScale(this.bIndex, 0)
          this.translateScale(this.index, 0)
        } else if (x0Offset > 0 && imgStatus.x >= allowX) {
          ...
          this.statusX = 3
          imgStatus.m = x0Offset
          if (imgPositionX >= allowX) {
            //     右滑到达左边界
            this.statusX = 4
            var overX = imgPositionX - allowX
            imgStatus.m =
              allowX - imgStatus.x + this.getSlowlyNum(overX, maxWidth)
          }
          if (imgPositionX <= -allowX) {
            //     左滑到达右边界
            this.statusX = 4
        ...
        //     scale == 1;
        if (Math.abs(y0Offset) > 5 && this.statusX != 5) {
          //     长图片处理
          var $img = this.getJqElem(this.index)
          var imgBottom = $img.height - this.winh
          if (y0Offset > 0 && imgPositionY > 0) {
            this.statusX = 7
            this.allowY = 0
            imgStatus.my =
              -imgStatus.y + this.getSlowlyNum(imgPositionY, maxWidth)
          } else if (y0Offset < 0 && imgPositionY < -imgBottom) {
            this.statusX = 7
            if ($img.height > this.winh) {
              var overY = imgPositionY + imgBottom
              this.allowY = -imgBottom
              imgStatus.my =
                -imgBottom - imgStatus.y + this.getSlowlyNum(overY, maxWidth)
            } else {
              this.allowY = 0
              imgStatus.my =
                -imgStatus.y + this.getSlowlyNum(imgPositionY, maxWidth)
          ...
          this.statusX = 5
          if (
            (this.index == 0 && x0Offset > 0) ||
            (this.index == this.maxLen && x0Offset < 0)
          ) {
            //     box到达左右边界
            this.box.m = this.getSlowlyNum(x0Offset)
          } else {
            this.box.m = x0Offset
          }
          this.translateScale(this.bIndex, 0)
  ...
  _previewImage.prototype.endAction = function(ts, te) {
    var imgStatus = this.getIndexImage()
    var x0Offset = te.x0 - ts.x0
    var y0Offset = te.y0 - ts.y0
    var time = te.time - ts.time
    var slipTime = 0
    this.allowMove = false //     结束所有行为
    if (ts.length == 1) {
      //     单手指(图片位移)
      if (Math.abs(x0Offset) > 10) {
        //     防止误触关闭看图
        event.preventDefault()
      }
      switch (this.statusY) {
      ...
      switch (this.statusX) {
        case 1: //     左滑->初始状态到达或超过右边界->图片平滑移动到达条件-切换下一图
          if (
            ...
            (x0Offset <= -this.imageChageNeedX ||
              (time < 200 && x0Offset < -30))
          ) {
            //     下一图
            this.changeIndex(1)
          } else {
        ...
        case 2: //     右滑->初始状态到达或超过左边界->图片平滑移动到达条件-切换上一图
          if (
            this.index != 0 &&
            (x0Offset >= this.imageChageNeedX || (time < 200 && x0Offset > 30))
          ) {
            //     上一图
            this.changeIndex(-1)
          } else {
  ...
  _previewImage.prototype.getAllow = function(index) {
    ...
    var allowX = Math.floor(
      ($img.width * imgStatus.scale - this.winw) / (2 * imgStatus.scale)
    )
    var allowY0, allowY1
    if ($img.height * imgStatus.scale <= this.winh) {
      allowY0 = 0
      ...
      allowY0 = Math.floor(
        ($img.height * imgStatus.scale - this.winh) / (2 * imgStatus.scale)
      )
      allowY1 = -allowY0
    } else {
      allowY0 = Math.floor(
        ($img.height * (imgStatus.scale - 1)) / (2 * imgStatus.scale)
      )
      allowY1 = -Math.floor(
        ($img.height * (imgStatus.scale + 1) - 2 * this.winh) /
          (2 * imgStatus.scale)
      )
    }
    return {
  ...
  _previewImage.prototype.getScale = function(ts, tm) {
    var fingerRangeS = Math.sqrt(
      Math.pow(ts.x1 - ts.x0, 2) + Math.pow(ts.y1 - ts.y0, 2)
    ) //     两手指的初始距离
    var fingerRangeM = Math.sqrt(
      Math.pow(tm.x1 - tm.x0, 2) + Math.pow(tm.y1 - tm.y0, 2)
    ) //     两手指移动过程中的距离
   ...
   */
  _previewImage.prototype.addCssPrefix = function(elem, prop, value) {
    //     可以优化todo
    for (var i in this.cssprefix) {
      ...
      if (cssprefix === '') {
        ...
        var len = prop.length
        prop =
          prop.substr(0, 1).toUpperCase() + prop.substr(1, len).toLowerCase()
      }
